...
export class AdminUsersEditComponent implements OnInit {
  ...
  ngOnInit() {
    this.selectedState(this.selectRow.stateorigin)  
    this.getRoles();
    this.addUserForm = this.formBuilder.group({
      'name':[this.selectRow.name, Validators.required],
      'password':[this.selectRow.password , Validators.required],
      'email':[this.selectRow.contactemail, Validators.compose([Validators.email,Validators.required])],
      'phone':[this.selectRow.contactmobile,  Validators.compose(
        [Validators.required, Validators.maxLength(11), Validators.minLength(10)])
      ],
      'gender':[this.selectRow.gender, Validators.required],
      'state':[this.selectRow.stateorigin, Validators.required],
      'lga':[this.selectRow.lga, Validators.required],
      'address':[this.selectRow.contactaddress, Validators.required],
      'work_id':[this.selectRow.work_id, Validators.required],
      // 'photograph':['', Validators.required],
      'accountno':[this.selectRow.accountno, Validators.required],
      'accountstatement':[this.selectRow.accountstatement, Validators.required],
      'accounttype':[this.selectRow.accounttype, Validators.required],
      'referral':[this.selectRow.referral],
      'status':['', Validators.required],
      'bvn':[this.selectRow.bvnverify],
      'reg':[this.selectRow.regCompleted],
      'active':[this.selectRow.active],
      'card':[this.selectRow.cardstatus],
      'roles':[this.selectRow.roles.id],
    })
  }

  selectedState(stateName){
    this.state = this.states;
    this.lga =  this.state.filter(
    (state) =>  state.state.name == stateName)[0].state.locals;
  }

  getRoles(){
    this.loading = true;
    this.data.usersService.getUserRoles()
  ...
  addUser(){
    ...
    let rolesArray = form.get('roles').value;

    this.selectRow.accountno = form.get('accountno').value;
    this.selectRow.accountstatement = form.get('accountstatement').value;
    this.selectRow.accounttype = form.get('accounttype').value;
    this.selectRow.referral = form.get('referral').value;
    this.selectRow.password = form.get('password').value;
    this.selectRow.contactemail = form.get('email').value;
    this.selectRow.contactaddress = form.get('address').value;
    this.selectRow.contactmobile = form.get('phone').value;
    this.selectRow.bvnverify = form.get('bvn').value == null ? false:true;
    this.selectRow.active = form.get('active').value == null ? false:true;
    this.selectRow.status = form.get('status').value;
    this.selectRow.stateorigin = form.get('state').value;
    this.selectRow.lga = form.get('lga').value;
    this.selectRow.cardstatus = form.get('card').value == null ? false:true;
    this.selectRow.gender = form.get('gender').value;
    this.selectRow.name = form.get('name').value;
    // this.selectRow.photograph = form.get('photograph').value;
    this.selectRow.workid = form.get('work_id').value;
    this.selectRow.regCompleted = form.get('reg').value == null ? false:true;

    this.data.usersService.createAdminUser(this.selectRow,rolesArray)
    .subscribe( () => {
      this.loading = false;
