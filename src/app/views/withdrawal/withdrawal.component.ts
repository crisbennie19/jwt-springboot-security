...
export class WithdrawalComponent implements OnInit {
  ...
  getWithdrawalList() {
    this.loading = true;
    this.data.withdrawal.getWithdrawalList(1, 1000).subscribe(
      (res: any) => {
  ...
  getWithdrawalByEmail() {
    ...
    this.data.withdrawal.getWithdrawalByEmail(this.email).subscribe(
      (res: any) => {
        if (res.message == "Success") {
          this.mydata = res.data;
          this.loading = false;
  ...
  getWithdrawalByDate() {
    ...
    this.data.withdrawal.getWithdrawlByDate(todate).subscribe(
      (res: any) => {
        if (res.message == "Success") {
          this.mydata = res.data;
          this.loading = false;
  ...
  approve(row) {
    ...
    this.data.withdrawal
      .getWithdrawal_aprove_decline(payload)
      .subscribe((res: any) => {
        if (res.message == "Success") {
          this.loading = false;
