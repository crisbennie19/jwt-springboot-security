...
export class CreditCardReportComponent implements OnInit {
  ...
  getDailyReportList() {
    ...
    if (this.todate == null || !this.reportOption) {
      ...
      this.data.reportService.getDailyCreditReport(day, month, year).subscribe((res: any) => {
        this.invalid = false
        this.loading = false
  ...
  getWeeklyReport() {
    ...
    this.data.reportService.getWeeklyCreditReport(this.m, this.y).subscribe((res: any) => {
      this.loading = false
      this.invalid = false
      this.tableLength = res.data.length
      this.listData = new MatTableDataSource(res.data);
  ...
  getQuartelyReport() {
    ...
    if (this.endquarter == this.startquarter) {
      ...
      this.data.reportService.getQuarterlyCreditReport(this.endquarter, this.y, this.startquarter).subscribe((res: any) => {
        this.invalid = false
        this.loading = false
        this.tableLength = res.data.length
        this.dataList =true
  ...
  getYearlyReport() {
    ...
    if (this.y == "") {
      ...
      this.data.reportService.getYearlyCreditReport(this.y).subscribe((res: any) => {
  ...
  if(this.download =="daily"){
    ...
    const day = date.date();
      const month = date.month() + 1;
      const year = date.year(); 
     this.data.reportService.getDailyCreditReportAttachment(0,day, month, year,"PDF", 100)
    .subscribe( res => {
      downloadFilePDF(res)
      this.loading = false;
    }, err => {
  ...
  if(this.download =="weekly"){
    this.loading = true;
     this.data.reportService.getWeeklyCredictAttachment(0,this.m, this.y,"PDF", 100)
    .subscribe( res => {
      downloadFilePDF(res)
      this.loading = false;
    }, err => {
  ...
  downloadXcel(){
    ...
    const day = date.date();
      const month = date.month() + 1;
      const year = date.year();
    this.data.reportService.getDailyCreditReportAttachment(0,day, month, year,"EXCEL", 100)
    .subscribe( res => {
      console.log(res)
