...
export class VcardReportComponent implements OnInit {
  ...
  getDailyReportList() {
    ...
    if (this.todate == null || !this.reportOption) {
      ...
      this.data.reportService.getDailyVCardReport(day, month, year).subscribe((res: any) => {
        this.invalid = false
        this.loading = false
  ...
  getQuartelyReport() {
    ...
    if (this.endquarter == this.startquarter) {
      ...
      this.data.reportService.getQuarterlyVCardReport(this.endquarter, this.y, this.startquarter).subscribe((res: any) => {
        this.invalid = false
        this.loading = false
        this.tableLength = res.data.length
        this.dataList =true
  ...
  getYearlyReport() {
    ...
    if (this.y == "") {
      ...
      this.data.reportService.getYearlyVCardReport(this.y).subscribe((res: any) => {
  ...
  downloadXcel(){
    ...
    const day = date.date();
      const month = date.month() + 1;
      const year = date.year();
    this.data.reportService.getDailyVCardAttachment(0,day, month, year,"EXCEL", 100)
    .subscribe( res => {
      console.log(res)
