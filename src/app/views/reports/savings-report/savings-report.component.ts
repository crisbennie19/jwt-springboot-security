...
@Component({
  ...
  styleUrls: ['./savings-report.component.scss']
})
...
export class SavingsReportComponent implements OnInit {
  ...
  displayedColumns = ['name','card', 'sunday', 'monday', 'tuesday', 'wednessday', 'thursday', 'friday', 'saturday'];
  ...
  getDailyReportList() {
    ...
    if (this.todate == null || !this.reportOption) {
      ...
      this.data.reportService.getDailySavingsReport(day, month, year).subscribe((res: any) => {
        this.invalid = false
        this.loading = false
  ...
  getWeeklyReport() {
    ...
    this.data.reportService.getWeeklySavingsReport(this.m, this.y).subscribe((res: any) => {
      this.loading = false
      this.invalid = false
      this.tableLength = res.data.length
      this.listData = new MatTableDataSource(res.data);
  ...
  getQuartelyReport() {
    ...
    if (this.endquarter == this.startquarter) {
      ...
      this.data.reportService.getQuarterlySavingsReport(this.endquarter, this.y, this.startquarter).subscribe((res: any) => {
        this.invalid = false
        this.loading = false
        this.tableLength = res.data.length
        this.dataList =true
  ...
  getYearlyReport() {
    ...
    if (this.y == "") {
      ...
      this.data.reportService.getYearlySavingsReport(this.y).subscribe((res: any) => {
  ...
  if(this.download =="weekly"){
    this.loading = true;
     this.data.reportService.getWeeklySavingsAttachment(0,this.m, this.y,"PDF", 100)
    .subscribe( res => {
      downloadFilePDF(res)
      this.loading = false;
    }, err => {
  ...
  if(this.download =="quartely"){
    this.loading = true;
     ...
     this.data.reportService.getQuartelySavingsReportAttachment(this.endquarter,0,this.y,"PDF", 100,this.startquarter)
    .subscribe( res => {
      downloadFilePDF(res)
      this.loading = false;
    }, err => {
  ...
  downloadXcel(){
    ...
    const day = date.date();
      const month = date.month() + 1;
      const year = date.year();
    this.data.reportService.getDailySavingsAttachment(0,day, month, year,"EXCEL", 100)
    .subscribe( res => {
